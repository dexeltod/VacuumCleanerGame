...
namespace UI.UI.GUI.HyperCasualAssets.GUI_Kit_Casual_Game.Extensions.UIParticle
{
	...
	public class UIParticleSystem : MaskableGraphic
	{
		...
		protected override void OnPopulateMesh(VertexHelper vh)
		{
			...
			int count = pSystem.GetParticles(
				particles
			);
 ...
 (pSystem.simulationSpace == ParticleSystemSimulationSpace.Local ? particle.position : _transform.InverseTransformPoint(particle.position));
				...
				Color32 color = particle.GetCurrentColor(
					pSystem
				);
					...
					switch (textureSheetAnimation.animation)
					{
						case ParticleSystemAnimationType.WholeSheet:
							frame = Mathf.FloorToInt(
								frameProgress * textureSheetAnimationFrames
							);
							break;
