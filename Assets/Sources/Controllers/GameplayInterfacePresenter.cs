	...
	public class GameplayInterfacePresenter : Presenter, IGameplayInterfacePresenter
	{
		...
		)
		{
			...
			_levelChangerService = levelChangerService ??
			                       throw new ArgumentNullException(
				                       nameof(levelChangerService)
			                       );
			_gameplayInterfaceView
				= gameplayInterfaceView ??
				  throw new ArgumentNullException(
					  nameof(gameplayInterfaceView)
				  );
			_speedDecorator = speedDecorator ??
			                  throw new ArgumentNullException(
				                  nameof(speedDecorator)
			                  );
			_coroutineRunnerProvider = coroutineRunnerProvider ??
			                           throw new ArgumentNullException(
				                           nameof(coroutineRunnerProvider)
			                           );
			if (time <= 0)
		...
		public void SetCashScore(int cashScore) =>
			_gameplayInterfaceView.SetCashScore(
				cashScore
			);
		...
		public override void Enable()
		{
			...
			IncreaseSpeedButton.onClick.AddListener(
				OnIncreaseSpeed
			);
		...
		public override void Disable()
		{
			...
			IncreaseSpeedButton.onClick.RemoveListener(
				OnIncreaseSpeed
			);
			GoToNextLevelButton.onClick.RemoveListener(
				OnGoToNextLevel
			);
		}
		...
		{
			_gameplayInterfaceView.FillSpeedButtonImage(
				0
			);
