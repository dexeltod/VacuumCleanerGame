...
namespace Sources.Infrastructure.Factories.StateMachine
{
    public class GameStateChangerFactory : Factory<IGameStateChanger>, IGameStateChangerFactory
    {
        ...
        {
            _gameStatesRepositoryFactory = gameStatesRepositoryFactory ?? throw new ArgumentNullException(nameof(gameStatesRepositoryFactory));
        }

        public override IGameStateChanger Create() => new GameStateChanger(new GameStateContainer(), _gameStatesRepositoryFactory.Create());
    }
}