...
using System.Linq;
using System.Text;
using SerializeInterfaces.Editor.UIElements;
using UnityEditor;
    ...
    internal class ObjectSelectorWindow : EditorWindow
    {
        ...
        private void FilterItems()
        {
            ...
            _filteredItems.AddRange(_allItems.Where(item =>
                ...
                item.Label.IndexOf(SearchText, StringComparison.InvariantCultureIgnoreCase) >= 0));
        ...
        private IEnumerable<ItemInfo> FetchAllAssets()
        {
            ...
            while (property.Next(null))
            {
                yield return new ItemInfo
                    { Icon = property.icon, InstanceID = property.instanceID, Label = property.name };
            }
                ...
                foreach (var comp in go.GetComponents(typeof(Component)))
                {
                    if (CheckFilter(comp))
                        yield return new ItemInfo
                        {
                            ...
                            InstanceID = comp.GetInstanceID(), Label = property.name
                        };
                }
            }
        }
