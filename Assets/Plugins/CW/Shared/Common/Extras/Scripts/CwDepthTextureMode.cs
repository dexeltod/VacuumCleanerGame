        ...
        public static void RequireDepth()
        {
            ...
            if (found == false)
            {
                ...
                if (Camera.main != null)
                {
                    ...
                    {
                        if (HelpButton("This component requires your camera to render a Depth Texture, but it doesn't.",
                                MessageType.Error, "Fix", 50.0f) == true)
                        {
                            CwHelper.GetOrAddComponent<CwDepthTextureMode>(Camera.main.gameObject).DepthMode =
                ...
                else
                {
                    ...
                    foreach (var camera in Camera.allCameras)
                    {
                        ...
                        {
                            CwHelper.GetOrAddComponent<CwDepthTextureMode>(camera.gameObject).DepthMode =
                                DepthTextureMode.Depth;
