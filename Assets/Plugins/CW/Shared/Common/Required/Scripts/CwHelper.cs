...
namespace Plugins.CW.Shared.Common.Required.Scripts
{
	...
	public static partial class CwHelper
	{
		...
		private static readonly int _CW_ShapeTex = Shader.PropertyToID(
			"_CW_ShapeTex"
		);

		private static readonly int _CW_ShapeCoords = Shader.PropertyToID(
			"_CW_ShapeCoords"
		);

		private static readonly int _CW_ShapeChannel = Shader.PropertyToID(
			"_CW_ShapeChannel"
		);

		private static readonly int _CW_ShapeColor = Shader.PropertyToID(
			"_CW_ShapeColor"
		);
		...
		public static T GetAssetImporter<T>(string path)
			where T : AssetImporter
		{
			return AssetImporter.GetAtPath(
				path
			) as T;
		}
