...
{
	/// <summary>
	///     This component will update a <b>TMP_Dropdown</b> component with localized text, or use a fallback if none is
	///     found.
	/// </summary>
	[ExecuteInEditMode]
	...
	[HelpURL(
		LeanLocalization.HelpUrlPrefix + "LeanLocalizedDropdown"
	)]
	[AddComponentMenu(
		LeanLocalization.ComponentPathPrefix + "Localized Dropdown"
	)]
	public class LeanLocalizedTMP_Dropdown : MonoBehaviour, ILocalizationHandler
	{
		[SerializeField] private List<Option> options;

		[NonSerialized] private HashSet<LeanToken> tokens;
		...
		public void UpdateLocalization()
		{
			...
			List<TMP_Dropdown.OptionData> dOptions = dropdown.options;
			...
			if (options != null)
				for (var i = 0; i < options.Count; i++)
				{
					...
					LeanTranslation stringTranslation = LeanLocalization.GetTranslation(
						option.StringTranslationName
					);

					// Use translation?
					if (stringTranslation != null && stringTranslation.Data is string)
						dOption.text = LeanTranslation.FormatText(
							(string)stringTranslation.Data,
							dOption.text,
							this,
							gameObject
						);
					...
					LeanTranslation spriteTranslation = LeanLocalization.GetTranslation(
						option.StringTranslationName
					);
		...
		public class Option
		{
			...
			[Tooltip(
				"If StringTranslationName couldn't be found, this text will be used"
			)]
			public string FallbackText;

			[Tooltip(
				"If SpriteTranslationName couldn't be found, this sprite will be used"
			)]
